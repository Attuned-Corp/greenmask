...
func Test_AutoAnonymize(t *testing.T) {
	...
	t.Run("AutoAnonymize enabled", func(t *testing.T) {
		...
		// The users table has id (PK, skipped by auto-anonymize) and username (explicit transformer),
		// so only the explicit transformer should be present.
		assert.Equal(t, 1, len(usersTable.TransformersContext), "Should have only the explicitly defined transformer since id is a PK")
		...
		assert.NotContains(t, transformerColumns, "id", "Should NOT have transformer for id column since it is a primary key")
	})
	...
	t.Run("SkipAutoAnonymize for specific columns", func(t *testing.T) {
		...
		// Should have transformer for username but NOT for id (PK and in SkipAutoAnonymize)
		...
		assert.Contains(t, transformerColumns, "username", "Should have transformer for explicitly defined username column")
		assert.NotContains(t, transformerColumns, "id", "Should NOT have transformer for id column (primary key)")
	})
